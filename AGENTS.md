...
The continuous-improvement loop (audit → fix → ship → verify) runs as **slash-skills** for Claude Code, or `mbe` CLI subcommands for other tools. Claude Code loads project skills from `.claude/skills/` (both project automation and general engineering advice). See [CLAUDE.md](./CLAUDE.md#continuous-improvement-loop) for the full skill catalog and scheduling.
...
- `llms.txt` — Rialto component catalog (UI patterns), regenerated by `pack-changed` post-commit hook.
