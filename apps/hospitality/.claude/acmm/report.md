# ACMM Scorecard — Level 2 · Instructed

_Generated 2026-04-26 · 11/85 criteria detected · role: **Rule-writer**_
...
> **Anti-pattern at this level:** Instruction rot: letting CLAUDE.md drift out of sync with the code so the rules stop matching reality.

> **Next transition trigger:** "I want the system to tune itself from the metrics."

## Since last run

**Since last run:** no change (still L2, 11 detected).

## Next steps — close 3 of 4 L3 gaps

Each line below is a concrete remediation hint derived from the criterion's detection paths. The cheapest path to L3 is to satisfy the 3 cheapest items.

- **`acmm:pr-acceptance-metric`** — PR acceptance tracking
  → `touch scripts/build-accm-history.mjs` (or any of: `.github/workflows/accm-history-update.yml`, `scripts/pr-metrics.mjs`)
- **`acmm:pr-review-rubric`** — PR review rubric
  → `touch .github/review-rubric.md` (or any of: `docs/review-criteria.md`, `.github/prompts/review.md`, `docs/qa/`)
- **`acmm:quality-dashboard`** — Quality dashboard
  → `touch web/public/analytics.js` (or any of: `web/src/components/analytics/`)
- **`acmm:ci-matrix`** — CI matrix
  → `touch .github/workflows/build.yml` (or any of: `.github/workflows/build-deploy.yml`, `.github/workflows/ci.yml`, `.github/workflows/test.yml`)
...
Prerequisites (L0, soft indicator): **3/8**

## By source framework

| Source | Detected | Total | Citation |
|---|---|---|---|
| [AI Codebase Maturity Model](https://arxiv.org/abs/2604.09388) | 10 | 65 | Anderson, A. (2026). The AI Codebase Maturity Model. arXiv:2604.09388 |
...
To advance from L2 → L3, close ≥70% of the criteria below.

- **`acmm:pr-acceptance-metric`** — PR acceptance tracking
  Scheduled job that tracks AI PR acceptance rate over time.
  _Detection:_ `scripts/build-accm-history.mjs` · `.github/workflows/accm-history-update.yml` · `scripts/pr-metrics.mjs`
- **`acmm:pr-review-rubric`** — PR review rubric
  Committed rubric the AI follows when reviewing PRs.
  _Detection:_ `.github/review-rubric.md` · `docs/review-criteria.md` · `.github/prompts/review.md` · `docs/qa/`
- **`acmm:quality-dashboard`** — Quality dashboard
  A dashboard or page that renders the AI loop metrics.
  _Detection:_ `web/public/analytics.js` · `web/src/components/analytics/`
- **`acmm:ci-matrix`** — CI matrix
  Matrix CI testing multiple platforms/versions.
  _Detection:_ `.github/workflows/build.yml` · `.github/workflows/build-deploy.yml` · `.github/workflows/ci.yml` · `.github/workflows/test.yml`
