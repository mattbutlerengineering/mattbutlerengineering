...
test.describe("CF-2: Timeline loads and displays reservations", () => {
  test("timeline page loads with grid", async ({ mockedPage }) => {
    ...
    // Scoped to the stats row and exact: a bare `getByText("Live")` is a case-insensitive
    // *substring* match, so any guest whose name contains "live" (Oliver, Clive, Olivera) makes
    // it resolve to two nodes and strict-mode-fail on a spec that has nothing to do with them.
    await expect(
      mockedPage.getByTestId("timeline-stats").getByText("Live", { exact: true })
    ).toBeVisible();
  ...
  test("late-evening clock (23:30 local) still shows today's fixture reservations", async ({
    ...
    await mockedPage.clock.setFixedTime(new Date(`${localDay()}T23:30:00`));
    await mockedPage.goto("timeline");
...
// The day is read inside each test (`localDay()`), never here: a `const` at describe scope is
// evaluated when Playwright collects the file, so a run that straddles local midnight computes
// one day at import and another at request time (#5279 item 3).
test.describe("Item 16: the now-line, scroll-to-now, and the quiet night", () => {
  ...
  test("P01: at 20:00 local the now-line reads 8:00 PM and the grid scrolls to it", async ({
    ...
    await mockedPage.clock.setFixedTime(new Date(`${localDay()}T20:00:00`));
    await mockedPage.goto("timeline");
  ...
  test("P01: at 14:00 local the now-line reads 2:00 PM, six hours to the left of 8:00 PM", async ({
    ...
    await mockedPage.clock.setFixedTime(new Date(`${localDay()}T14:00:00`));
    await mockedPage.goto("timeline");
  ...
  test("no now-line on another date; Today brings it back and re-scrolls to it", async ({
    ...
    await mockedPage.clock.setFixedTime(new Date(`${today}T20:00:00`));
    await mockedPage.goto(`timeline?date=${localDay(1)}`);
    ...
    await mockedPage
      ...
      .click();
    await expect(mockedPage).toHaveURL(new RegExp(`[?&]date=${today}`));
