const CRITERIA = [
  {
    ...
    description:
      "A mechanism that captures user corrections during agent sessions and persists them.",
    rationale:
      "Claude-reflect: corrections are the highest-signal feedback an agent receives — discarding them means repeating the same mistakes.",
    details:
      'Correction capture is a system that records every time you correct the AI during a session — "no, not that way" or "use X instead of Y" — and saves it to a persistent file. Without it, the AI forgets your corrections between sessions and you end up repeating yourself. An AI mission will set up a reflections directory and memory files that automatically persist corrections for future sessions.',
    detection: {
      type: "any-of",
      pattern: [
        ".claude/reflections/",
        "memory/feedback_",
        ".github/ai-corrections.yml",
        "scripts/capture-corrections.mjs",
      ],
    },
  },
  {
  ...
  },
  {
    ...
    description:
      "A workflow that syncs captured corrections/preferences into CLAUDE.md or AGENTS.md.",
    rationale:
      "Claude-reflect: the feedback loop only closes when learned preferences flow back into the config the next agent session reads.",
    details:
  ...
  },
  {
    ...
    rationale:
      "Claude-reflect: an unstructured pile of reflections becomes unsearchable fast — an index keeps lookups cheap.",
    details:
      "A preference index is a structured file (JSON, YAML, or Markdown with frontmatter) that organizes captured preferences by topic, file area, or category so the AI can quickly look up what you prefer for a specific situation. Without an index, reflections become an unsearchable pile that the AI either loads entirely (wasting context) or ignores. An AI mission will create a MEMORY.md index that categorizes your existing preferences.",
    detection: {
      type: "any-of",
      pattern: [".claude/preferences.json", "memory/MEMORY.md", ".github/agent-preferences.yml"],
    },
  },
  {
    ...
    rationale:
      "Claude-reflect: reflections decay in relevance; without review they become noise that the agent then trusts.",
    details:
    ...
    detection: {
      type: "any-of",
      pattern: [
        ".github/workflows/reflection-review.yml",
        "scripts/review-reflections.mjs",
        "docs/reflection-review.md",
      ],
    },
  },
  {
    ...
    rationale:
      "Claude-reflect: session summaries are the unit of learning — they bridge one session's context to the next.",
    details:
      "A session summary artifact is a file written at the end of each AI session recording what was changed, what approaches were tried (including failed ones), and what was learned. It bridges context between sessions so the next session can pick up where the last one left off instead of re-discovering the same things. An AI mission will add a session summary template and post-session hook that captures this information automatically.",
    detection: {
      type: "any-of",
      pattern: [".claude/sessions/", "docs/session-summaries/", "memory/session_"],
    },
  },
];
