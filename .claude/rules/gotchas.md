...
- **PreToolUse secret-scan hook (`.claude/hooks/secret-scan.mjs`)** shifts the CI Gitleaks scan left to edit time: on `Edit|Write|MultiEdit` it scans the content an agent is about to write for high-confidence secrets (Stripe `sk_live_`/`pk_live_`/`rk_live_`, AWS `AKIA`/`ASIA` key ids, JWTs, PEM private-key headers) and **blocks the write (`exit 2`)** on a hit, before the secret ever reaches a commit. Detection is the pure, unit-tested `scanForSecrets()` in `scripts/secret-scan.mjs` (test-mode keys, `.example`/`.sample`, `__tests__`/`fixtures`, and `.md` are allowlisted to keep false positives near zero); the hook fails **open** (`exit 0` on any parse error) so a hook bug never deadlocks editing. Wiring the hook modifies agent startup config (`.claude/settings.json`), so it is a **one-time human step** — add this block to the `PreToolUse` array:
  ```json
  { "matcher": "Edit|Write|MultiEdit", "hooks": [ { "type": "command", "command": "node \"$CLAUDE_PROJECT_DIR/.claude/hooks/secret-scan.mjs\"", "timeout": 5 } ] }
  ```
