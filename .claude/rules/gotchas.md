...
- **Removing a workspace package requires auditing all service Dockerfiles** — orphaned `COPY packages/<removed>/…` lines fail Docker builds with a file-not-found error at build time. After removing or renaming a package, run `grep -r "<pkg-name>" services/*/Dockerfile` to find stale COPY references.
...
- **`ignoreChanges: ["spec"]` on the DO App Pulumi resource silently blocks new env vars** — new env vars added to `infrastructure/pulumi/index.ts` are never pushed by `pulumi up` for that component. Bridge the gap by injecting the missing var via `yq` + `doctl apps update` in `deploy-services.yml`, or accept that removing `ignoreChanges` lets `doctl`-injected metadata get overwritten on the next Pulumi run.
- **ACMM `isWorkflowActive` must query `--status=success`, not `--status=completed`** — workflows that skip on passing deploys (e.g. `auto-rollback.yml` skips on every non-rollback) flood `--status=completed --limit=5` with skipped entries, crowding real successes out of the window and making the detector falsely report the workflow as inactive. Querying `--status=success` returns only confirmed-green runs.
- **`gh issue create --label <name>` exits 1 if the label doesn't exist in the repo** — the CLI returns `could not add label: '<name>' not found` and fails the entire workflow step even when the issue body is valid. Pre-create all labels referenced in automated workflows (`gh label create <name> --color <hex>`) or set labels in a follow-up step that can fail gracefully without blocking issue creation.
- **GH Actions workflows that hit the live site must validate required secrets at startup** — a missing bypass token (e.g. `AUDIT_TOKEN`) produces silent Cloudflare 403 responses with no meaningful error output; audit results come back empty for days with no visible CI failure. Add a `[ -z "${TOKEN:-}" ] && { echo "::error::TOKEN not set"; exit 1; }` guard as the first step so the job fails immediately with an actionable annotation instead of silently producing nothing.
...
- **Docker builder stage must use `--ignore-scripts` when services have `postinstall: prisma generate`** — the builder copies only `package.json` files before running install, so the postinstall fires without the Prisma schema present and fails with "Could not find Prisma Schema". Use `pnpm install --frozen-lockfile --ignore-scripts` in the builder stage; the explicit `db:generate` step that runs after the schema is copied handles client generation. Local and CI installs (full tree present) run postinstall normally and don't need this flag.
- **Service Dockerfiles must COPY `tsconfig.build.json` alongside `tsconfig.json`** — if the build step references a separate `tsconfig.build.json` (used to exclude test files from the compiled output), omitting it from the Docker COPY causes the build to fail. Pattern: `COPY services/<svc>/tsconfig.json services/<svc>/tsconfig.build.json ./services/<svc>/`
