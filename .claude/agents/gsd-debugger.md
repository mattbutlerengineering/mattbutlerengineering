...
Check code says:  hooksDir = path.join(configDir, 'hooks')
                  configDir = ~/.claude
                  → checks ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/hooks/

Installer says:   hooksDest = path.join(targetDir, 'hooks')
                  targetDir = ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done
                  → writes to ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/hooks/
...
```bash
INIT=$(node "${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/bin/gsd-tools.cjs" state load)
...
```bash
node "${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
...
```bash
node "${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs: update debug knowledge base with {slug}" --files .planning/debug/knowledge-base.md
```
