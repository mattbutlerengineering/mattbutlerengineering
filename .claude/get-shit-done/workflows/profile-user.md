...
- @${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/ui-brand.md (display patterns)
- @${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/agents/gsd-user-profiler.md (profiler agent definition)
- @${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/user-profiling.md (profiling reference doc)
  </required_reading>
...
```bash
PROFILE_PATH="${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/USER-PROFILE.md"
...
```bash
cp "${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/USER-PROFILE.md" "${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/USER-PROFILE.backup.md"
```
...
✓ Stores profile at ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/USER-PROFILE.md
...
```bash
SCAN_RESULT=$(node ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/user-profiling.md`
...
Reference: @${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/user-profiling.md
...
```bash
QUESTIONS=$(node ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  ...
  - "Global CLAUDE.md" -- "Add profile to ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/CLAUDE.md for all projects"

**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/USER-PROFILE.md" and jump to step 10.
...
```bash
node ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /gsd:dev-preferences at ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/commands/gsd/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
node ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/CLAUDE.md"
...
```bash
BACKUP_PATH="${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/USER-PROFILE.backup.md"
```
...
Your profile:    ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/USER-PROFILE.md
```
...
```
Artifacts:
  ✓ /gsd:dev-preferences   ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/commands/gsd/dev-preferences.md
  ✓ CLAUDE.md section       ./CLAUDE.md
  ✓ Global CLAUDE.md        ${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/CLAUDE.md
```
