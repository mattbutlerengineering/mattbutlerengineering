...
- **Global scope**: `${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/notes/`)
...
2. Glob `${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/notes/*.md` (if directory exists) — global notes
...
Global (${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `${PROJECT_ROOT:-$(git rev-parse --show-toplevel)}/.claude/notes/` — works in any directory
