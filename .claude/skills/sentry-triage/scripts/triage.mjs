...
if (!SENTRY_TOKEN) { console.error('SENTRY_ACCESS_TOKEN not set'); process.exit(1); }
...
async function ghApi(endpoint, options = {}) {
  const response = await fetch(`https://api.github.com${endpoint}`, {
    ...options,
    headers: { 'Authorization': `Bearer ${GITHUB_TOKEN}`, 'Accept': 'application/vnd.github.v3+json', ...options.headers }
  });
  return response.json();
}
...
async function triage() {
  ...
  const filtered = allIssues.filter(i => (i.level === 'error' || i.level === 'fatal') && parseInt(i.count) >= SEVERITY_THRESHOLD);
  console.log(`Filtered (>${SEVERITY_THRESHOLD} events): ${filtered.length}\n`);
  
  if (filtered.length === 0) { console.log('No actionable issues found. System is healthy!'); return { created: 0, skipped: 0, found: 0 }; }
  ...
  for (const issue of filtered.slice(0, 3)) {
    ...
    if (existing.total_count > 0) { skipped++; continue; }
    const byTitle = await searchExistingIssues(title);
    if (byTitle.total_count > 0) { skipped++; continue; }
    ...
    const result = await createIssue(`fix(${issue.project}): ${title}`, body, ['ready', 'sentry', 'bug']);
...
triage().then(r => console.log(`\nTriage complete: ${r.created} created, ${r.skipped} skipped of ${r.found} found`)).catch(e => { console.error('Error:', e.message); process.exit(1); });
