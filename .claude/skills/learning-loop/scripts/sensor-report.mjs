...
try {
  const prs = safeRun(
    'gh pr list --state all --limit 20 --json createdAt,mergedAt,state --jq "[.[] | select(.mergedAt != null)]"',
    "[]"
  );
...
try {
  ...
  const readyIssues = safeRun(
    "gh issue list --state open --label ready --json number --jq length",
    "0"
  );
...
const hasSentry =
  fs.existsSync("packages/sentry") ||
  safeRun('grep -r "sentry" packages/*/package.json 2>/dev/null', "") !== "";
