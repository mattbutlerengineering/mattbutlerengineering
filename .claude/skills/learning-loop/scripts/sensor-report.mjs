...
try {
  const ciStatus = safeRun(
    'gh run list --limit 10 --json conclusion,status --jq "[.[] | select(.status==\"completed\") | .conclusion]"',
    "[]"
  );
  ...
  report.sensors.ci = {
    available: true,
    passRate: runs.length > 0 ? ((passes / runs.length) * 100).toFixed(1) : 100,
...
try {
  const prs = safeRun(
    'gh pr list --state all --limit 20 --json createdAt,mergedAt --jq "[.[] | select(.mergedAt != null)]"',
    "[]"
  );
...
try {
  report.sensors.issues = {
    available: true,
    open: parseInt(safeRun("gh issue list --state open --json number --jq length", "0")) || 0,
    ready:
      parseInt(
        safeRun("gh issue list --state open --label ready --json number --jq length", "0")
      ) || 0,
  };
  report.summary.available++;
} catch {
...
if (hasInventory) {
  try {
    ...
    report.sensors.lighthouse = {
      ...
      surfacesTotal: inv.surfaces?.length || 0,
      note: checked > 0 ? `${checked}/${inv.surfaces?.length || 0} checked` : "needs first run",
    };
  } catch {
    report.sensors.lighthouse = { available: true, note: "parse error" };
  }
  report.summary.available++;
} else {
  report.sensors.lighthouse = { available: false, note: "no inventory" };
}
...
if (SENTRY_TOKEN) {
  try {
    const response = await fetch(
      "https://sentry.io/api/0/organizations/mattbutlerengineering/issues/?statsPeriod=7d&limit=10",
    ...
    if (response.ok) {
      ...
      report.sensors.sentry = {
        ...
        errorCount,
        note: errorCount === 0 ? "healthy" : `${errorCount} errors`,
      };
      report.summary.available++;
    } else {
      report.sensors.sentry = { available: false, note: "api error" };
    }
  } catch {
    report.sensors.sentry = { available: false, note: "network error" };
  }
} else {
  report.sensors.sentry = { available: false, note: "no token" };
}
...
if (hasSpend) {
  try {
    ...
    report.sensors.agentCost = { available: true, note: "parse error" };
    report.summary.available++;
  }
} else {
  report.sensors.agentCost = { available: false, note: "no spend log" };
}
