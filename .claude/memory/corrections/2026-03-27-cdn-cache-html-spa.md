...
feeds_back_into: CLAUDE.md#manual-deployment, .claude/rules/gotchas.md#deploy--static-sites
---
...
Lesson re-verified as still true and still enforced; the bare
`.claude/rules/gotchas.md` reference was tightened to the precise section
(`#deploy--static-sites`), which is where the constraint actually lives.

Worth noting for the next reviewer: `CLAUDE.md#manual-deployment` resolves,
but that section documents only the `wrangler deploy` commands — it does not
state the Workers-not-Pages constraint itself (measured: zero mentions of
"Pages" or "CDN"). Kept because it is where a reader deploying by hand would
look, not because it carries the rule.
