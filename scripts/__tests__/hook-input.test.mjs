...
describe("hook seam: Bash hooks read the command from the stdin payload", () => {
  ...
  it(
    ...
    () => {
      ...
      // the "pnpm install never ran" state. The warning must surface, and the
      // commit itself must not be blocked (WARN, not BLOCK — see
      // scripts/check-hooks-active.mjs's header).
      const work = gitFixtureAheadOfOrigin();
      const result = run("pre-bash-guard.sh", bashPayloadFor("git commit -m more"), work);
      expect(result.stderr).toContain("Git hooks are inert");
  ...
  it(
    "pre-bash-guard stays silent before `git push` once hooks are active",
    () => {
      const work = gitFixtureAheadOfOrigin();
