...
// `auth0.Branding` carries less of the Tenant drift risk above: the live
// tenant has never had a Branding record configured (stock Auth0 palette +
// badge, no custom logo) -- so there is no pre-existing manually-set value
// for `colors`/`logoUrl` that declaring them here could reset. Hex values
// are literal because Auth0 can't read CSS custom properties; they mirror
// rialto's DARK theme tokens (the login page should read as the app's dark
// speakeasy surface, not light mode) -- see
// packages/rialto/src/tokens/colors.css `[data-theme="dark"]`:
