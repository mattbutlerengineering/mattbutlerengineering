...
export class NotificationDispatcher {
  ...
  /**
   * Booking confirmation — transactional, always sent via email regardless of
   * channel. It carries the iCal attachment, which cannot travel via SMS, so
   * even a "sms" (or "both") resolved channel still delivers via email only.
   * The channel is accepted (not a raw preference) so callers thread the same
   * resolveChannel() result used for reminders — see booking-notifications.ts.
   */
  async sendBookingConfirmation(
    emailInput: BookingNotificationInput,
    _channel: "email" | "sms" | "both"
  ): Promise<void> {
