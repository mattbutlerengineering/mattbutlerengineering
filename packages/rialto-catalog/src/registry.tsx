...
function withAliasNormalization<T extends Record<string, AdapterFn>>(adapters: T): T {
  ...
  for (const [name, adapter] of Object.entries<AdapterFn>(adapters)) {
    wrapped[name] = (context) => adapter({ ...context, props: applyAliases(name, context.props) });
  }
...
export const { registry, handlers, executeAction } = defineRegistry(catalog, {
  components: withAliasNormalization(
    withSchemaValidation({
      ...
      Tabs: ({ props }: any) => <Tabs tabs={props.tabs} defaultTab={props.defaultTab} />,

      Breadcrumb: ({ props }: any) => <Breadcrumb items={props.items} maxItems={props.maxItems} />,
      ...
      Accordion: ({ props }: any) => <Accordion items={props.items} multiple={props.multiple} />,
