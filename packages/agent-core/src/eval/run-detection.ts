...
/**
 * A task that reports 0 turns and $0 cost never actually executed through
 * the agent — most commonly because the adapter had no credentials (e.g.
 * missing `ANTHROPIC_API_KEY`). A genuine run, even one that scores 0%,
 * always burns at least one turn or some cost.
 */
