...
function normalizeAbsolutePaths(typeText: string, repoRoot: string): string {
  ...
  function getSkeleton(node: Node, repoRoot = ""): string {
    ...
    if (Node.isClassDeclaration(node)) {
      ...
      const members = node.getMembers().slice(0, 5).map(m => {
    ...
    if (Node.isTypeAliasDeclaration(node)) {
      ...
      if (type && Node.isUnionTypeNode(type)) {
        const types = type.getTypeNodes().map(t => t.getText().substring(0, 30)).join(" | ");
    ...
    if (Node.isEnumDeclaration(node)) {
      const name = node.getName() || "Anonymous";
      const members = node.getMembers().slice(0, 10).map(m => `${m.getName()}${m.getValue() !== undefined ? " = " + m.getValue() : ""}`).join(", ");
...
function detectPriority(statement: Node): "high" | "medium" | "low" {
    ...
    const highKeywords = ["interface", "type ", "enum ", "export function", "export const", "export class", "export async function"];
  ...
  async function packDirectory(targetPath: string, root: string, forceFull = false, checkOnly = false): Promise<void> {
    ...
    if (forceFull) {
      ...
      console.log(`Successfully generated full context (${fullSizeKB.toFixed(1)}KB) at: ${skeletonPath}`);
    } else {
      writeFileSync(skeletonPath, skeletonOutput);
      writeFileSync(fullPathOutput, fullOutput);
      
      if (skeletonSizeKB > SIZE_LIMIT_KB) {
        console.warn(`  ⚠️  Skeleton exceeds ${SIZE_LIMIT_KB}KB limit: ${skeletonSizeKB.toFixed(1)}KB`);
      }
      
      if (fullSizeKB > AUTO_SPLIT_THRESHOLD_KB) {
        console.warn(`  ⚠️  Full context (${fullSizeKB.toFixed(1)}KB) split into llms.txt + llms-full.txt`);
      }
      
      console.log(`  ✓ llms.txt (${skeletonSizeKB.toFixed(1)}KB) + llms-full.txt (${fullSizeKB.toFixed(1)}KB)`);
    }
  }
