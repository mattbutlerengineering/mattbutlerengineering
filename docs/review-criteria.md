...
- **Trust the commit hooks.** Pre-commit runs `eslint --fix` + `check-adr`, post-commit runs `pack-changed`. If a PR's tests pass and ADR check passes, the _style_ is fine — focus on substance.
