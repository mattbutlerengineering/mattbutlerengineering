...
- [x] **Component page "Overlaps" section** — second `<Section>` on `TapeChartPage.tsx`, two stacked charts over the fixture, code sample, shared selection card
...
- [x] **Full gates and llms regen** — both packages green, `llms.txt` / `llms-full.txt` regenerated from committed-clean sources, `pnpm regen --check` clean
  - Accept:
...
- [x] **Commit, push, open the draft PR** — one conventional commit series staged by explicit path, pushed to `feat/tape-chart-overlaps`, draft PR against `main` to trigger the Linux visual job
  - Accept:
...
- [x] **Baseline round-trip from the Linux CI artifact** — commit `light-tape-chart-overlaps.png` (new) and `light-tape-chart-stress.png` (changed) taken from the failing Linux run, never from macOS
  - Accept:
...
- 2026-08-21 (Implement, item 1): the immutability test (`does not mutate its inputs and returns fresh bar objects`) already passed against the mutating `assignLanes` — the per-room bar lists were created inside the memo, so the input `reservations` array was never the one sorted in place. It is kept as the regression guard for the pure `packRoom`; the RED evidence for item 1 is the `laneCountByRoom` extension of the existing lane test.
- 2026-08-21 (Implement, item 2): `packRoom` spreads `overlap: kinds[i]` onto every bar, so a non-overlapping bar carries the key with value `undefined` rather than omitting it. Equivalent under `toBeUndefined()`, `data-overlap` (React drops `undefined` attributes) and the strings lookup; noted because the breakdown said "absent/undefined".
- 2026-08-21 (Implement, item 6): `pnpm build` also added a `TapeChartOverlapKind` entry (props `[]`, slots `[]`) to `registry.json` — the generator lists every type exported from the barrel (`TapeChartPositionedBar`, `TapeChartLayout` already appear the same way). Not in the breakdown's acceptance text; generator behaviour, not a choice.
- 2026-08-21 (Implement, items 1–5): the `regen-llms.sh` PostToolUse hook regenerated and staged `packages/rialto/llms.txt` / `llms-full.txt` after each rialto edit, so those two files travelled in the milestone-1 commits instead of waiting for item 12. Item 12 still runs the full `pnpm regen` + `pnpm regen --check` from a clean tree as the authority.
- 2026-08-21 (Implement, item 9): `pnpm --dir packages/rialto test:storybook` runs `Overlaps` and `Overlaps Classified` green (772 ms / 459 ms) but the suite exits 1 on 14 failures in other components' stories (Alert `Dismissible`, Avatar `With Image` / `Group`, and others — `TestingLibraryElementError`). None touch TapeChart; not verified against `main` (the suite is not in CI). Item 9 is ticked on the TapeChart plays plus `pnpm typecheck` / `pnpm lint`, per its acceptance text.
- 2026-08-21 (Implement, item 12): running the rialto gates, the rialto-web gates and `pnpm regen` concurrently starved all three (test timeouts of 5–33 s on generator/interaction tests that pass alone; `pnpm regen` exceeded 10 minutes and left a partial root `llms*.txt`, reverted). Re-run serially — see gotchas § CI on cold, parallel load tipping marginal-timeout suites.
- 2026-08-21 (Implement, item 12, result): serial `pnpm regen` completed ("Done. All artifacts regenerated.") and `pnpm regen --check` reported "All generated artifacts are up to date." Only root `llms.txt` / `llms-full.txt` changed — `packages/rialto/*` and `apps/rialto-web/*` llms files were already current from the hook, and `generated-schemas.ts` / `dep-graph.json` / `dependency-graph.md` are untouched. The root pack's per-file extract keeps the first two declarations of `useTapeChartLayout.ts`, so `llms.txt` now lists `OverlapClassifier` + `worstOf` where it listed `assignLanes` + the hook — generator behaviour, not edited by hand. Gates: rialto lint 0 errors, typecheck clean; `pnpm test` under a concurrent 83 %-CPU foreground process on the host gave 2043 passed / 109 failed, every failure a 5 s timeout or its `Axe is already running` cascade (no TapeChart file among them); the 10 affected files re-run with `--testTimeout=120000` pass 168/168, so the suite is green end to end. rialto-web lint 0 errors, typecheck clean, test 46 files / 589 passed.
- 2026-08-21 (Implement, item 13): the pre-push AI-antipattern ratchet (`scripts/check-ai-antipatterns.mjs`, `unusedParams` 4 → 5) flagged the `OVERLAP_CLASSIFIER_SAMPLE` string in `TapeChartPage.tsx` — the ux.md § code sample's `(a, b) =>` where `b` is unused and the letter occurs nowhere else in the file. Sample now reads `(a, _b) =>` (and the page test's regex with it); the rendered snippet deviates from ux.md by that underscore, which is also the honest signature for a classifier that ignores its second argument.
- 2026-08-21 (Implement, item 13, result): draft PR #4442 opened against `main` (title per the orchestrator brief: `feat(rialto): render tape chart overlaps as per-row lanes with conflict state`, not the breakdown's wording), head `aad81f284`, no auto-merge. CI fired on the real `pull_request` event (no `gate-missing`). First run: `Functional (rialto-web)` success; `CI Gate` **failure** because `Build › Run Full Repository Audit` (`pnpm check:prettier`) flagged the four pipeline-authored run-record docs (architecture/autorun-brief/idea/ux) — formatted with the repo `.prettierrc.js` in `a93a5f167`, content unchanged. `Visual Regression (rialto-web)` failed on **six** tests, not the predicted two: `light / tape-chart-overlaps` (missing baseline, as expected) plus `dark / dark-buttons`, `dark-inputs`, `dark-alerts`, `dark-avatar`, `dark-tape-chart` — each a ±1 px image-height change (113→114, 228→227, 319→320, 115→116, 640→639) whose diff pixels lie only on the harness's fixed `Dialog`/`Drawer` demo overlays, i.e. the sub-pixel clip shift documented in gotchas § CI (every dark section sits below the new `tape-chart-overlaps` section; `main`'s run 32525432785 on `443e2c474` has all of them green; `light / tape-chart-default` is pixel-identical; actuals are byte-identical across the CI retry). Not a TapeChart geometry regression, so item 5 was not reopened; the five dark baselines are refreshed from the same Linux artifact under item 14.
- 2026-08-21 (Implement, item 13, stress): `light / tape-chart-stress` **passed** on CI and locally even though room-1003 now renders two lanes (jsdom: `data-lane-count="2"`, Daniil Patel lane 0 / Yuki Singh lane 1, both `data-overlap="conflict"`; a scratch-config local render shows the 93 px row and rows 1004–1007 shifted, 1008 clipped). The committed baseline still shows the single-lane row with Yuki Singh hidden behind the dialog — `playwright.config.ts`'s `maxDiffPixelRatio: 0.01` (≈ 8.4 k px of 1232 × 685) absorbs a 45 px row growth on this sparse grid. Consequence for item 14: CI never writes an actual for a passing test, so the stale stress baseline is deleted in this push to force the Linux run to emit `light-tape-chart-stress-actual.png`; committing the macOS render is not an option. **For Verify/Review:** the visual gate's tolerance is loose enough to miss a whole-row layout change on the stress section — worth a follow-up (`maxDiffPixels` or a tighter ratio), outside this run's scope.
- 2026-08-21 (Implement, item 14, result): second Linux run 32543173746 (head `7a4de20b7`): `CI` success, `Functional (rialto-web)` success, `Visual Regression (rialto-web)` 48 passed / 1 failed — only `light / tape-chart-stress` (baseline absent by design, see item 13 note); `light / tape-chart-overlaps`, `light / tape-chart-default`, `dark / dark-tape-chart` and the four refreshed dark sections all green. `light-tape-chart-stress-actual.png` from that run's `rialto-web-visual-diffs` artifact is committed as the baseline: room-1003 is a 93 px two-lane row with Daniil Patel (lane 0) and Yuki Singh (lane 1) both conflict-red, rows 1004–1007 shift down 45 px, 1008 is clipped by the chart's `max-block-size: 70vh`; `room-1009` (Jan 15–21 / Jan 23–26, disjoint) unchanged. Two-step deviation from the "exactly those two PNGs in one commit" acceptance text: the overlaps + five dark baselines landed in `7a4de20b7` (from run 32540738717) and the stress baseline in the next commit (from run 32543173746), because CI only emits an actual for a failing test. **For Verify/Review:** the previous stress baseline labelled its first column "Wed 14" for `startDate="2026-01-15"` — identical to a macOS render of the same page — while Linux renders "Thu 15"; it was very likely a macOS-rendered baseline that the same 1 % tolerance hid. The new file is Linux-rendered.
