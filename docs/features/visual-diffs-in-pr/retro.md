...
assumptions:
  ...
  - "RESOLVED 2026-08-26, after this retro was written. This stage declined to restate the close-out brief's claim that F1 was found independently by two reviewers, because no artifact recorded a second reviewer. The claim was accurate about what happened but unevidenced in the repo: an adversarial reviewer was dispatched alongside the Review stage and returned only to the orchestrator's transcript. Its record — verdict `flag` 4/10, its own F1 reproduction, the four findings it uniquely contributed, and its evidence-based correction of F6 — is now appended to `review.md` as *Second review pass*. The refusal is left standing here because it is what surfaced the gap, and because the *Change* entry it produced (artifacts record findings, not provenance) is the durable lesson."
