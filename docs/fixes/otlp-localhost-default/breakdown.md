...
- [ ] **Wire `initTelemetry` to the plan using the empty-collections shape** — edit `packages/observability/src/sdk.ts` to fill `spanProcessors` / `traceExporter` / `metricReaders` from the plan per `architecture.md`'s mode table.
