...
5. COMMIT + PR. Conventional Commits. The COMMIT message body must contain `Closes #<ISSUE>` — a squash merge closes the issue from the commit trailer, not from the PR body, so a PR-body reference alone will not close it. Push the branch and open a PR targeting `main` whose title ENDS with the suffix `(mbe-daily-issue #<ISSUE>)` — e.g. `fix(hospitality): guard the empty floor-plan render (mbe-daily-issue #5321)`. Keep the rest of the title a normal Conventional Commits description; the suffix is the only thing that makes this routine's PRs distinguishable from ordinary implement-queue traffic, so its liveness can be verified at all (issue #5605). Label the PR `has-pr`, and run `mbe issue transition <ISSUE> --to has-pr`.
