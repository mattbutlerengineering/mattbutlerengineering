...
1. Run /implement-queue for one iteration with a batch of at most 3 independent ready issues (Phase 0 pre-flight through Phase 4). First step in every worktree: pnpm install --frozen-lockfile. Respect the circuit breaker; stop after one iteration. Before finishing, if metrics/queue-telemetry.jsonl has uncommitted appended rows, commit only that path on a branch and open a PR titled "chore(metrics): midday queue telemetry <date>" labeled has-pr — the "midday" prefix keeps this routine's PR distinguishable from mbe-night's identical step, so each routine's liveness can be verified independently (issue #5608).
