...
Human decisions are the factory's real throughput ceiling. List open issues labeled `ready-for-human`, `needs-review`, `blocked`, `agent-failed`, or `stealable` — this label-based GitHub query is the **membership** source for this pass; keep it, it is not what's broken. Anything untouched for more than 7 days is a flow blocker.

**Never sort this list, or compute its age, from `updatedAt`.** `stale-human-blocked.yml` runs Sunday 15:23 UTC, ahead of this retro, and *writes to the issues it measures* — it applies `ready-for-human` to whichever issues it finds stale. A label write bumps GitHub's `updatedAt` on exactly the issues that are most in need of surfacing, so `updatedAt` selectively erases the staleness signal for the most-ignored issues in the backlog and none of the others. This bit for real on the detector's first run, 2026-08-16: #3253 (37 days genuinely stale), #3597, and #3657 all looked touched-today and sorted last (#4274). Do not "simplify" this pass back to an `updatedAt` sort — that reintroduces the exact bug this pass exists to avoid.

Instead, get ordering and age from `metrics/stale-human-blocked.jsonl`: take the rows from the **most recent `detected_at` run** (the file is append-only across weekly runs; group by `detected_at` and use only the batch with the latest timestamp — do not mix rows from older runs), and sort those by `days_stale` descending. Use the row's `days_stale` as-is; it was computed from `last_human_touch_at` (a value label writes cannot move, by construction — it's derived from issue creation, comments, and non-label timeline events) at detection time and stays valid even though the label write that followed bumped `updatedAt` days ago.

An issue that shows up in the label query but has no row in the latest JSONL batch (e.g. it hadn't crossed the detector's own staleness threshold yet) is still reported — list it with its age explicitly marked **unknown**. Do not fall back to computing its age from `updatedAt`, and do not silently drop it from the pass. If `metrics/stale-human-blocked.jsonl` doesn't exist at all yet, report every issue from the label query with age unknown rather than falling back to an `updatedAt` sort for the whole pass.
